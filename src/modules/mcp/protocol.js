...
import { POWER_PLATFORM_AUTHORING, CONTINUATION_INSTRUCTIONS, DATAVERSE_BULK_SCHEMA_GUIDANCE } from './power-platform-authoring.js';
  ...
  if (body.method === 'initialize') {
    ...
    return sendMcpJson(ctx, 200, { jsonrpc: '2.0', id: body.id, result: {
      ...
      instructions: CONTINUATION_INSTRUCTIONS + '\n'
        + (!isGateway && !isDevOps && !isSharePoint && !isPowerPages ? POWER_PLATFORM_AUTHORING + '\n' + DATAVERSE_BULK_SCHEMA_GUIDANCE + '\n' : '')
        + (isGateway && (connection.gatewayDomains || []).includes('dataverse') ? DATAVERSE_BULK_SCHEMA_GUIDANCE + '\n' : '')
