...
import { resolveTarget, fetchFiles, requireCapability, WORKSPACE_ID_DESCRIPTION } from './shared.js';
...
export function registerWriteTools(server, ctx) {
  server.registerTool(
    'ensure_gitignore',
    {
      title: 'Keep dependency output out of Git',
      description:
        'Plans or applies safe `.gitignore` rules for the project on the user\'s machine. Use this ' +
        'whenever you scaffold a project, add a package manifest, create dependency/build folders, ' +
        'or before running an install, restore, build, test, or generator command.\n\n' +
        'With `apply: false` (the default), this is a read-only preview: it returns every missing rule ' +
        'and why it is needed. With `apply: true`, it appends those rules to one Quicker Portal managed ' +
        'block. Existing `.gitignore` lines are never removed or reordered, the edit is approval-bound ' +
        'and undoable, and a Git safety checkpoint is taken first.\n\n' +
        'DEPENDENCY POLICY: package manifests and lockfiles such as package.json, package-lock.json, ' +
        'pnpm-lock.yaml, yarn.lock, *.csproj, packages.lock.json, pyproject.toml, requirements.txt, ' +
        'Cargo.toml and Cargo.lock are reproducibility source and remain tracked. Downloaded dependencies ' +
        'and generated output such as node_modules, .venv, bin, obj, target, dist and caches are ignored. ' +
        'Never add a lockfile to `.gitignore` merely because it was generated by an install.',
      inputSchema: {
        workspaceId: z.string().optional().describe(WORKSPACE_ID_DESCRIPTION),
        mode: z
          .enum(['project', 'observed'])
          .optional()
          .describe(
            '`project` (recommended) detects ecosystems from manifests and protects folders before they ' +
              'exist. `observed` only proposes rules for dependency/build artifacts already on disk.'
          ),
        paths: z
          .array(z.string())
          .max(500)
          .optional()
          .describe(
            'Relative paths just created or about to be created. Include generated folders such as ' +
              'node_modules/pkg, packages/api/obj, .venv, dist, or target when known.'
          ),
        apply: z
          .boolean()
          .optional()
          .describe('False previews exact additions. True applies them through the desktop approval and undo path.'),
        reason: z
          .string()
          .optional()
          .describe('Why upkeep is needed, shown to the user when approval is requested.')
      },
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false }
    },
    toolHandler('ensure_gitignore', async (args, extra) => {
      ...
      const { workspace, agent, session, clientName } = await resolveTarget(ctx, extra, args.workspaceId, {
        toolName: 'ensure_gitignore',
        requireLiveAgent: true,
        summary: args.apply ? 'Apply dependency ignore rules' : 'Review dependency ignore rules'
      });
      requireCapability(agent, 'ensureGitignore');

      if (workspace.kind !== 'folder') {
        return ok('This is a single-file workspace, so there is no project `.gitignore` to maintain.', {
      ...
      const paths = [...new Set((args.paths || []).map((value, index) => normalizeRelPath(value, `paths[${index}]`)))];
      const mode = args.mode === 'observed' ? 'observed' : 'project';
      const request = {
        workspaceId: workspace.id,
        mode,
        paths,
        apply: false,
        meta: { actor: 'mcp', actorName: clientName, reason: args.reason || 'dependency-hygiene', at: Date.now() }
      };

      let result = await agent.request(AGENT_METHOD.ENSURE_GITIGNORE, request, {
        timeoutMs: config.bridgeRpcTimeoutMs
      });
      if (!result?.ok) {
        return fail(result?.message || result?.error || 'The desktop app could not inspect `.gitignore`.', result);
      }

      let baseline = null;
      if (args.apply && result.additions?.length) {
        baseline = await checkpointBeforeChanges(ctx, { workspace, agent, clientName });
      ...
      if (write?.ok) {
        workspace.applyChange({
          type: write.action === 'create' ? 'created' : 'updated',
          path: '.gitignore',
          entry: {
            path: '.gitignore',
            size: write.size,
            revision: write.revision,
            mtime: write.mtime ?? Date.now(),
            binary: false,
            dirty: false
          },
          revision: write.revision,
          ...
          summary: args.reason || 'Maintain dependency and build ignore rules'
        });
        ctx.contentCache.dropFile(workspace.id, '.gitignore');
        session.noteWrite(workspace.id, '.gitignore', write.revision);
        workspace.verification.markDirty(['.gitignore']);
        ctx.hub.notifyUser(workspace.userId, SERVER_EVENT.CHANGES_APPLIED, {
          workspaceId: workspace.id,
          clientName,
          summary: 'Updated .gitignore for dependencies and generated output',
          paths: ['.gitignore'],
          verification: workspace.verification.toJSON()
        });
      } else if (args.apply && result?.write && !result.write.ok) {
        return fail(
          `The proposed .gitignore rules were not applied: ${result.write.message || result.write.error || 'desktop refusal'}`,
          result
        );
      }

      const additions = result?.additions || [];
      const ecosystemNames = (result?.ecosystems || []).map((item) => item.name || item.id).filter(Boolean);
      const lines = additions.map((item) => `  ${item.pattern}${item.why ? `  - ${item.why}` : ''}`);
      const text = [];
      if (result.applied) {
        text.push(`Updated .gitignore with ${additions.length} missing rule(s).`);
      } else if (additions.length) {
        text.push(
          `Found ${additions.length} missing .gitignore rule(s):`,
          '',
          ...lines,
          '',
          args.apply
            ? 'The project changed while applying; inspect again before running dependency-producing commands.'
            : 'PLAN ONLY - call ensure_gitignore again with apply: true before installing dependencies or building.'
        );
      } else {
        text.push('Dependency and generated-output ignore rules are already current; no file change is needed.');
      }
      if (ecosystemNames.length) text.push('', `Detected: ${ecosystemNames.join(', ')}.`);
      text.push(
        '',
        'Existing .gitignore rules were preserved. Dependency manifests and lockfiles remain tracked; only downloaded dependencies and generated/local output are ignored.'
      );
      const baselineNote = describeCheckpoint(baseline, { baseline: true });
      if (baselineNote) text.push('', baselineNote);

      return ok(text.join('\n'), {
        workspaceId: workspace.id,
        mode,
        hasGitignore: Boolean(result?.hasGitignore),
        ...
        write: write?.ok
          ? { path: '.gitignore', action: write.action, revision: write.revision, size: write.size }
          : null,
        verification: write?.ok ? workspace.verification.toJSON() : undefined
      });
    })
  );
  ...
  server.registerTool(
    'write_files',
    {
      ...
      description:
        ...
        'When this call scaffolds a project or creates a dependency manifest, call `ensure_gitignore` ' +
        'with mode "project" and apply true before running install/build commands. Keep package manifests ' +
        'and lockfiles tracked; ignore downloaded dependencies and generated output.\n\n' +
